...
class StoppingDistanceComputer:
    ...
    def __init__(self, traj_int, *, proj_mass=1837, max_step=np.inf, rtol = 1e-5, atol = 1e-7):
    ...
    def compute_stopping_distance(self, start_point, start_velocity, *, stop_velocity_mag = 0.4, max_time = 3e5, output = None, status = True):
        ...
        rk = RK45(fun, 0, [v_init, 0], max_time, max_step = self.max_step, rtol = self.rtol, atol = self.atol)
        ...
        while rk.y[0] > stop_velocity_mag:
            ...
            if output is not None and i % output == 0:
                ...
                if status:
                    rk.t_old = 0 if i == 1 else rk.t_old  

                    print(f'Step: {i} - Time: {rk.t} - Velocity: {rk.y[0]} - Position: {rk.y[1]} - time step: {rk.t - rk.t_old} cpu time: {perf_counter() - cts:0.4f} sec - wall time: {time() - wts:0.4f} sec ', end="\r")
